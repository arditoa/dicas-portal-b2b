...
export default function FeaturedTourismScreen() {
  const router = useRouter();

  const trips: VIPTrip[] = [
    {
      id: 'trip_hero',
      title: 'Pousada Arco-Íris & Resort',
      tagline: 'Sua experiência pé na areia com atendimento acolhedor, piscina aquecida e coquetelaria.',
      category: 'Hospedagem Exclusiva',
      location: 'Florianópolis • SC',
      benefit: 'Late Check-out & Drink de Boas-Vindas',
      rating: '5.0',
    ...
    {
      id: 'trip2',
      title: 'Roteiro Baixo Augusta Queer',
      tagline: 'Guia completo dos melhores pubs, teatros independentes e restaurantes parceiros.',
      category: 'Roteiro Urbano',
      location: 'São Paulo • SP',
      benefit: 'Mapa de descontos exclusivo',
      rating: '4.9',
      image: 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=800&q=80',
    },
    {
      id: 'trip3',
      title: 'Eco Lodge & Spa Chapada',
      tagline: 'Imersão na natureza com chalés privativos, passeios guiados e gastronomia sustentável.',
      category: 'Ecoturismo VIP',
      location: 'Chapada dos Veadeiros • GO',
      benefit: '15% OFF na segunda diária',
      rating: '4.9',
      image: 'https://images.unsplash.com/photo-1501339847302-ac426a4a7cbb?w=800&q=80',
    },
  ];

  const heroTrip = trips.find((t) => t.isHero);
  const regularTrips = trips.filter((t) => !t.isHero);
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.heroTitleArea}>
            <View style={styles.vipBadge}>
              <Feather name="compass" size={12} color={COLORS.safeSpace} />
              <Text style={styles.vipBadgeText}>TURISMO & DICAS TRIP</Text>
            </View>
            <Text style={styles.mainTitle}>Destinos Selecionados</Text>
            <Text style={styles.subTitle}>Curadoria oficial dos locais de hospedagem e passeios parceiros.</Text>
          </View>
          ...
          {heroTrip && (
            <TouchableOpacity
              ...
              onPress={() => router.push(`/(tabs)/tourism` as any)}
              activeOpacity={0.9}
            >
              <ImageBackground source={{ uri: heroTrip.image }} style={styles.heroImage} imageStyle={{ borderRadius: 20 }}>
                <View style={styles.heroGradient}>
                  <View style={styles.heroTopBadges}>
                    <View style={styles.masterTag}>
                      <Feather name="award" size={10} color="#000" />
                      <Text style={styles.masterTagText}>SELO DICAS TRIP</Text>
                    </View>
                    <View style={styles.ratingTag}>
                      <Feather name="star" size={10} color={COLORS.gold} />
                      <Text style={styles.ratingTagText}>{heroTrip.rating}</Text>
                    </View>
                  </View>

                  <View style={styles.heroBottomContent}>
                    <Text style={styles.heroCategory}>{heroTrip.category}</Text>
                    <Text style={styles.heroTitle}>{heroTrip.title}</Text>
                    <Text style={styles.heroTagline} numberOfLines={2}>{heroTrip.tagline}</Text>
                    
                    <View style={styles.benefitBox}>
                      <Feather name="gift" size={12} color={COLORS.gold} />
                      <Text style={styles.benefitText}>{heroTrip.benefit}</Text>
                    </View>
                  </View>
                </View>
              </ImageBackground>
          ...
          <View style={styles.listSection}>
            <Text style={styles.sectionHeaderTitle}>Roteiros & Experiências Recomendadas</Text>

            {regularTrips.map((item) => (
              <TouchableOpacity
                key={item.id}
...
const styles = StyleSheet.create({
  ...
  vipBadge: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(76, 175, 125, 0.15)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, alignSelf: 'flex-start', marginBottom: 8, borderWidth: 1, borderColor: 'rgba(76, 175, 125, 0.3)' },
  vipBadgeText: { fontSize: 10, fontWeight: '800', color: COLORS.safeSpace, letterSpacing: 1 },
  ...
  heroCategory: { fontSize: 11, fontWeight: '700', color: COLORS.safeSpace, textTransform: 'uppercase' },
  heroTitle: { fontSize: 22, fontWeight: '800', color: '#FFF' },
  heroTagline: { fontSize: 12, color: '#D0D0E0', lineHeight: 17 },
  benefitBox: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(255, 213, 79, 0.25)', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, marginTop: 6, alignSelf: 'flex-start' },
  ...
  partnerCategory: { fontSize: 10, fontWeight: '700', color: COLORS.safeSpace },
